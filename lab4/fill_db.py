...
class DatabaseFiller:
    ...
    def fill_buyers_table(self, count=15):
        ...
        for _ in range(count):
            buyer_data = (self.fake.company(), 'Russia', self.fake.address()[:200], self.fake.numerify('#########'), random.choice(categories))
            cursor.execute("""
    ...
    def fill_employee_table(self, count=20):
        ...
        for _ in range(count):
            ...
            clean_fio = f"{last_name} {first_name} {middle_name}"
    ...
    def fill_bonus_manager_table(self, count=20):
        ...
        if not self.invoice_pos:
            logging.warning("Список invoice_pos пуст. Нечего добавлять в бонусы.")
            return
