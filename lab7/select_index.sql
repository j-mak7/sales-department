...
-- DROP INDEX idx_invoice_product;
DROP INDEX idx_invoice_payment;

-- CREATE INDEX idx_invoice_product
-- ON invoice_pos (id_invoice, article_number)
-- INCLUDE (quantity, price);

CREATE INDEX idx_invoice_payment
ON invoice (date_payment)
WHERE payment_stamp = TRUE;
...
prod_per_month AS (
	SELECT 	p.article_number, p.name_product, EXTRACT (MONTH FROM i.date_payment) AS month_payment,
			SUM (ip.quantity) AS total_quantity,
	...
	JOIN invoice_pos ip ON ip.article_number = p.article_number
	JOIN invoice i ON ip.id_invoice = i.id_invoice
	WHERE i.date_payment >= '2025-01-01' AND i.date_payment < '2026-01-01'
...
DROP INDEX idx_invoice_pos_q_p;
DROP INDEX idx_course_money_unit;
DROP INDEX idx_invoice;

CREATE INDEX idx_invoice_pos_q_p ON invoice_pos (id_invoice)
INCLUDE (quantity, price);

CREATE INDEX idx_course_money_unit ON course_money_unit (id_unit, date_course)
INCLUDE (rate_value);

-- CREATE INDEX idx_invoice
-- ON invoice (id_invoice) 
-- INCLUDE (id_buyer, id_unit, date_register, payment_stamp, cancell_stamp);
